...
from model.model_init_utils import init_model_opt_scaler_from_dataset
...
class BaseDataset(ABC):
    ...
    def load_data(dataset_name):
        path = Path(__file__).parent / "datasets" / dataset_name
    ...
    @staticmethod
    def filter_column_types(self, df):
...
class ColumnEncodingDataset:
    def __init__(self, args, dataset):
        ...
        self.masks, self.mode_masks = None, None
    ...
    def run_preprocessing(self):
        ...
        for data_dict in self.dataset_gen:
            ...
            self.encode(data_dict["data_table"])
    ...
    def create_masks(self, range_indices):
        ...
        bert_mask_matrix = ~(self.train_mask_matrix | self.val_mask_matrix |
                             self.test_mask_matrix | self.dataset.missing_matrix)
        mode_mask_matrix = ~bert_mask_matrix

        self.mode_masks = {key: (mode_mask_matrix, bert_mask_matrix) for key in ["train", "val", "test"]}
        self.masks = {key: matrix for key, matrix in
                         zip(("train", "val", "test"), (self.train_mask_matrix, self.val_mask_matrix, self.test_mask_matrix))}
    ...
    def encode(self, data_table):
        ...
        for col_idx in range(self.dataset.D):
            ...
            if col_idx in self.dataset.cat_features:
                fitted_enc = OneHotEncoder(sparse=False).fit(enc_fit_col) # avoid using "?" as label
                encoded_col = fitted_enc.transform(column)
            else:
                fitted_enc = StandardScaler().fit(enc_fit_col)
                encoded_col = (fitted_enc.transform(column)).reshape(-1, 1)
    ...
    def mask_according_to_mode(self, dataset_mode, row_index_order):
        ...
        to_mask_mode, to_mask_bert = self.mode_masks[dataset_mode]
        ...
        return to_mask_input, masked_tensors, label_mask_matrix, augmentation_mask_matrix, target_loss_matrix
    ...
    def apply_mask(data_arrs, mask_candidates, cat_features, mask_prob, args):
        ...
        # model is always semi supervised
        if mask_prob == 1:
...
class IncomeDataset(BaseDataset): # binary classification
    def __init__(self, c):
        ...
        self.num_target_cols = []
        # self.data_table = self.missing_numerical(df)
...
class MiniadultDataset(IncomeDataset):
    def __init__(self, c):
        super().__init__(c)
        self.c = c
        dm = self.data_table[self.data_table[:, -1] == '>50K'][:8, :]
        db = self.data_table[self.data_table[:, -1] == '<=50K'][:8, :]
        self.data_table = np.concatenate([dm, db], 0)
        self.N, self.D = self.data_table.shape
        self.missing_matrix = np.zeros((self.N, self.D), dtype=np.bool_)
        self.cat_target_cols = [self.D - 1]
...
class NPTDataset(torch.utils.data.IterableDataset):
    ...
    def __next__(self):
        ...
        return input_batch, masked_batch, target_loss_matrix, label_mask_matrix, augmentation_mask_matrix
...
def main(args):
    ...
    return wandb_args, args, wandb_c, wandb_run

if __name__ == "__main__":

    parser = build_parser()
    args = parser.parse_args()
    dataset = find_dataset(name=args.data_set)
    data = dataset(args)
    # data = MinicancerDataset(args)
    # data = MiniadultDataset(args)

    dataset = ColumnEncodingDataset(args, data)
    wandb_args, args, wandb_c, wandb_run = main(args)
    starting_cv_index = 0
    total_n_cv_splits = int(1 / args.exp_test_perc)

    for cv_index in range(starting_cv_index, total_n_cv_splits):
        print(f'\nCV Index: {cv_index}')
        print(f'Train-test Split {cv_index + 1}/{total_n_cv_splits}')
        dataset.load_next_cv_split()
        print("\n\ninput_arrs, masked_arrs, label_mask, augm_mask, target_loss matrices of train, val, test in bytes:",
              sys.getsizeof(dataset.model_input), "bytes\n")
        print("dataset")
        print("args.exp_device", args.exp_device)
        model, optimizer, scaler = init_model_opt_scaler_from_dataset(dataset=dataset, c=wandb_c, device=args.exp_device)
        u = Hi(wandb_c)
        # trainer = Trainer(model=model, optimizer=optimizer, scaler=scaler,
        #                   args=wandb_c, wandb_run=wandb_run, cv_index=cv_index, dataset=dataset)

        # trainer.train_and_eval()
        # dataset_mode = "train"
        # tensor_dataset = NPTDataset(dataset.model_input, "train", batch_size=4)
        # dataloader = torch.utils.data.DataLoader(dataset=tensor_dataset, batch_size=4,
        #                                          shuffle=False,
        #                                          num_workers=4)
