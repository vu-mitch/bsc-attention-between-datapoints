    ...
    * Loss.finalize_epoch_losses() to obtain the loss_dict for the current
    ...
    def compute_loss(
        ...
        if label_mask_matrix is None:
            ...
            loss_indices['label'] = data_dict[f'target_loss_matrix']
        ...
        target_cols = self.metadata["cat_target_cols"] + self.metadata["num_target_cols"]
        # Compute losses per column
        for col, (out, dat) in enumerate(zip(output, ground_truth_data)):
            is_cat = col in self.metadata["cat_features"] #data_dict['cat_features']
            ...
            for mode, mode_loss_indices in loss_indices.items():
                ...
                if mode == 'label' and col not in target_cols:
                    continue
