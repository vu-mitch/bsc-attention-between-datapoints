...
import wandb
from model.image_patcher import LinearImagePatcher
from model.utils.npt_utils import MHSA
...
class NPTModel(nn.Module):
    ...
    def build_hybrid_enc(self, enc, AttentionBlocks, att_args, D):
        # enc: [ReshapeToFlat()], attentionblock: cycle(MHSA), att_args: {'c': self.c}
    ...
    def forward(self, X_ragged, X_labels=None, eval_model=None):
        ...
        # from NxDxE back to NxD example: N=4, D=31, E=64 back to N=4, D=31
        # len(X_ragged[0:]) = 31 ; X_ragged[1].shape = 4 x 1
        # X_ragged[0].shape = 4 x 2, label encoding shape
