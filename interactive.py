...
from model.preprocess import ColumnEncodingDataset, find_dataset
from model.init_model_opt_scaler import init_model_opt_scaler
...
def build_parser():
    ...
    parser.add_argument(
        ...
        help='Path of data')
    parser.add_argument(
        '--data_set', type=str, default='breastcancer',
        help=f'Currently supported are breastcancer, poker-hand, '
        f'higgs, epsilon, forest-cover, minicancer, mnist, yacht, '
        f'concrete, income, protein, cifar10.')
    parser.add_argument(
        '--data_loader_nprocs', type=int, default=-1,
    ...
    parser.add_argument(
        '--exp_n_runs', type=int, default=10,
    ...
    parser.add_argument(
        '--exp_val_perc', type=float, default=0.2,
        help='Percent of examples in validation set')
    parser.add_argument(
        '--exp_test_perc', type=float, default=0.1,
        help='Percent of examples in test set. '
             'Determines number of CV splits.')
    parser.add_argument(
        '--exp_num_total_steps', type=float, default=500,
    ...
    parser.add_argument(
        '--exp_eval_every_n', type=int, default=1,
    ...
    parser.add_argument(
        ...
        help='The number of steps over which we warm up. This is only used '
             ...
             'description.')
    parser.add_argument(
        '--exp_lr', type=float, default=5e-4,
        help='Learning rate')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        f'other rows. Need to set dataset=debug. '
        f'If MODE == crossword: '
...
if __name__ == "__main__":
    ...
    data = dataset(args)
    from model.preprocess import MinicancerDataset, MiniadultDataset
    ...
    for cv_index in range(starting_cv_index, total_n_cv_splits):
        ...
        # init model and optimizer only need metadata
        model, optimizer, scaler = init_model_opt_scaler(c=wandb_c, metadata=dataset.metadata, device=args.exp_device)
        ...
        # print("number of samples:", N, "number of batches: ", np.ceil(N / batch_size))
        # tensor_dataset = NPTDataset(dataset.model_input, "val", batch_size=batch_size)
        # print("len(tensor_dataset)", len(tensor_dataset))
        # dataloader = torch.utils.data.DataLoader(dataset=tensor_dataset,
        #                                          shuffle=False,
        #                                          num_workers=0,
        #                                          collate_fn=collate_with_pre_batching,
        #                                          drop_last=True)

        # print("shape of input array", dataset.model_input["train"][0][1].shape)
        # for batch_idx, batch in enumerate(dataloader):
        #     if batch["input_arrs"][0].shape[0] == 0:
        #         break
        #     print("row_idx", batch["row_idx"])
        #     print("batch_idx dataloader", batch["batch_idx"])
        #     print(batch["input_arrs"][1].shape)
        #     # if batch["input_arrs"][0].shape[0] == 0:
        #     #     break
